...
public class DocumentService {
  ...
  public DocumentDownload downloadDocument(String id) {
    ...
    GridFSFile gridFsFile =
        gridFsTemplate.findOne(
            Query.query(Criteria.where("_id").is(new ObjectId(document.getGridFsId()))));
    if (gridFsFile == null) {
      throw new DocumentNotFoundException(id);
    }
  ...
  private String storeFile(MultipartFile file) {
    try (InputStream inputStream = file.getInputStream()) {
      ObjectId objectId =
          gridFsTemplate.store(inputStream, file.getOriginalFilename(), file.getContentType());
  ...
  private DocumentDTO mapToDto(Document document) {
    DocumentDTO.DocumentDTOBuilder builder =
        DocumentDTO.builder()
            .id(document.getId())
