...
public class FolderControllerIntegrationTest {
  ...
  void createListUpdateAndDeleteFolder() throws Exception {
    ...
    String createResponse =
        mockMvc
            .perform(
                post("/api/folders").contentType(MediaType.APPLICATION_JSON).content(createJson))
    ...
    mockMvc
        .perform(get("/api/folders/" + folderId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.name").value("Renamed Projects"));

    mockMvc
        .perform(delete("/api/folders/" + folderId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true));

    mockMvc.perform(get("/api/folders/" + folderId)).andExpect(status().isNotFound());
  }
}
